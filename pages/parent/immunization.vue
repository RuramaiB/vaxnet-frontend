    ...
    <div class="flex gap-8 h-[calc(100vh-220px)]">
      ...
      <div class="w-80 flex-shrink-0 flex flex-col gap-4">
        ...
        <div class="flex-1 bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden flex flex-col">
          <div class="p-6 border-b border-gray-50">
            <h3 class="font-bold text-gray-900">My Children</h3>
          </div>
      ...
      <div class="flex-1 overflow-y-auto custom-scrollbar rounded-[2.5rem] bg-[#f9f7f2] border-4 border-white shadow-2xl relative">
        ...
        <div v-if="!selectedChild" class="h-full flex flex-col items-center justify-center p-12 text-center">
          ...
          <h2 class="text-2xl font-bold text-gray-900">Health Card Not Selected</h2>
          <p class="text-gray-500 mt-2 max-w-xs mx-auto">Please select a child from your directory to view their official "Road to Health" digital card.</p>
        </div>
                ...
                >
                  ...
                  <td class="p-6">
                    <div v-if="dose.status !== 'ADMINISTERED'" class="flex items-center gap-2">
                       <span class="text-[10px] text-gray-400 font-bold uppercase tracking-widest italic">Awaiting Clinical Admin</span>
                    </div>
...
const fetchChildren = async () => {
  ...
  try {
    const resp = await fetch(`${API_BASE}/children/get-children-by-parent-email/${parentEmail.value}`)
    if (resp.ok) {
      ...
      const targetBCN = route.query.child as string
      if (targetBCN) {
        const target = children.value.find(c => c.birthCertificateNumber === targetBCN)
        if (target) selectChild(target)
